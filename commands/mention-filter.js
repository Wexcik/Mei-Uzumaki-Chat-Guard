...
exports.run = async (client, message, args, prefix, dil, database) => {
  ...
  if (args[0] === "on" || args[0] === "aç") {
    if (sunucu.etiketKoruma) return message.reply(dil == "tr" ? "Bu özellik zaten sunucuda aktif!" : "This filter is already active on this server!");
    await database.updateOne({ guildID: message.guild.id }, { etiketKoruma: true }, { upsert: true });
    message.channel.send(dil == "tr" ? "Filtre aktif edildi!" : "Filter has been activated!");
    return;
  };
  
  if (args[0] === "off" || args[0] === "kapat" || args[0] === "kapa") {
    if (!sunucu.etiketKoruma) return message.reply(dil == "tr" ? "Bu özellik zaten sunucuda devre dışı!" : "This filter is already deactive on this server!");
    await database.updateOne({ guildID: message.guild.id }, { etiketKoruma: true }, { upsert: true });
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ['etiket-engel', 'etiket', 'mention', 'mentionfilter', 'etiketengel', 'etiket-filtresi', 'etiket-koruma'],
...
exports.help = {
  name: 'mention-filter',
  description: 'Mention filter.',
  usage: 'mention-filter on/off',
  isim: 'etiket-engel',
  aciklama: 'Etiket filtresi.',
  kullanim: 'etiket-engel aç/kapat',
  category: 'filtre'
};