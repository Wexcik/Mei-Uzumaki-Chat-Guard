...
exports.run = async (client, message, args, prefix, dil, database) => {
  ...
  if (args[0] === "on" || args[0] === "aç") {
    if (sunucu.linkKoruma) return message.reply(dil == "tr" ? "Bu özellik zaten sunucuda aktif!" : "This filter is already active on this server!");
    await database.updateOne({ guildID: message.guild.id }, { linkKoruma: true }, { upsert: true });
    message.channel.send(dil == "tr" ? "Filtre aktif edildi!" : "Filter has been activated!");
    return;
  };
  
  if (args[0] === "off" || args[0] === "kapat" || args[0] === "kapa") {
    if (!sunucu.linkKoruma) return message.reply(dil == "tr" ? "Bu özellik zaten sunucuda devre dışı!" : "This filter is already deactive on this server!");
    await database.updateOne({ guildID: message.guild.id }, { linkKoruma: false }, { upsert: true });
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ['link-engel', 'link', 'linkfilter', 'linkengel', 'link-filtresi'],
...
exports.help = {
  ...
  usage: 'link-filter on/off',
  isim: 'link-engel',
  aciklama: 'Link/bağlantı filtresi.',
  kullanim: 'link-engel aç/kapat',
  category: 'filtre'
};